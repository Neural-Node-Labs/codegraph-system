...
|---|---|---|
| `search_code` | `query: str, limit: int=25` | Full-text search over node names/signatures/docstrings. Start here to find a node id. |
| `get_node` | `node_id: int` | Full details for one node (file, line, signature, docstring). |
| `list_nodes` | `type?: str, file_path?: str, limit: int=100` | List/filter nodes by type or exact file. |
| `list_relations` | `type?: str, resolved?: bool, query?: str, limit: int=200` | Flat list of edges with source/target names joined in — inspect every relation directly. |
| `get_dependencies` | `node_id: int, depth: int=1` | What this node depends on, outward N hops. |
| `get_dependents` | `node_id: int, depth: int=1` | What depends on this node, inward N hops. |
| `impact_of_change` | `node_id: int, depth: int=2` | Blast radius if this node changes — use before editing shared code. |
| `find_path` | `source_id: int, target_id: int, max_depth: int=6` | Shortest dependency path between two nodes. |
| `list_unresolved` | `limit: int=100` | Edges static analysis couldn't resolve (dynamic imports, external packages, template routes). |
| `get_stats` | — | Node/edge counts by type; quick sanity check on graph coverage. |
| `refresh_mapping` | — | Re-run static analysis and rebuild the graph. Call after the codebase changes. |

Verified against a real MCP stdio session: tool discovery and calls to
`get_stats`, `search_code`, and `refresh_mapping` all round-trip correctly
against a live codegraph backend.

### Example agent flow

```
1. search_code("payment")
   -> finds candidate nodes, e.g. node_id 142 (PaymentService.charge)
2. get_dependencies(142, depth=2)
   -> see what it imports/reads/calls
3. impact_of_change(142, depth=3)
...
- Every codegraph user has a personal `api_key`. The MCP server sends it as
  ...
  the web UI (member vs. admin). `refresh_mapping` is available to any
  authenticated user, not just admins, since re-indexing is read-triggering,
  not destructive.
...
- `refresh_mapping` is a **blocking** call — for large repositories it may
